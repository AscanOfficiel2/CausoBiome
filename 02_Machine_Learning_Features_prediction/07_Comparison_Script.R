...
df <- read.csv("CRC_model_foldwise_scores_with_metadata.csv")
names(df) <- make.names(names(df))

# Reshape to long format
df_long <- df %>%
  pivot_longer(cols = c(Accuracy, F1.Macro, Kappa, MCC),
               names_to = "Metric", 
               values_to = "Score")
...
# === Save to TIFF ===
tiff("model_comparison_plot_with_metadata.tiff", width = 8, height = 6, units = "in", res = 600, compression = "lzw")
