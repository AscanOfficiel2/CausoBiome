...
Original file is located at
    https://colab.research.google.com/drive/1WaT-uhSm_t003D2qdthKQF4eg7D2oM3d
"""
...
# Confounders (Age, BMI, Sex, and Country)
meta["Sex"] = meta["Sex"].astype(str).str.lower().map({"male": 1, "m": 1, "female": 0, "f": 0})

base_confounders = meta[["Age", "BMI", "Sex"]].apply(pd.to_numeric, errors="coerce")
base_confounders = base_confounders.fillna(base_confounders.median())

# One-hot encode 'Country'
country_dummies = pd.get_dummies(meta["Country"], prefix="Country", dummy_na=False)

# Combine numeric and one-hot categorical confounders
Xconf = pd.concat([base_confounders, country_dummies], axis=1).values
print(f"✅ Confounders included: Age, BMI, Sex, and {country_dummies.shape[1]} Country dummy variables")
